<template>
  <v-dialog v-model="open" fullscreen eager attach="#library" scrollable>
    ...
    <v-card dark class="mx-auto" :loading="loading">
      ...
      <transition name="up">
        <div id="bottom" v-show="showBottom">
          ...
          <span class="mt-5">{{ `${pageNum}/${read.totalPages}` }}</span>
...
export default {
  ...
  methods: {
    ...
    next() {
      if (this.pageNum < this.read.totalPages && !this.pageRendering)
        this.pageNum++;
    },
    ...
    async prepareReading() {
      try {
        ...
        this.scale = this.read.zoom;
        this.slider = this.pageNum = this.read.page;
      } catch (err) {
        console.error(err);
      } finally {
        this.loading = false;
    ...
    async update() {
      ...
      read.lastRead = new Date().toISOString();
      read.page = this.pageNum;
      read.zoom = this.scale;
      await this.updateRead(read);
    },
    close() {
      this.open = false;
      this.exitFullscreen();
